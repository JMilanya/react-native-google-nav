 ...
 * ```
 */
export async function optimizeWaypointOrder(options) {
  const {
    apiKey,
    origin,
    waypoints,
  ...
  if (travelMode === TravelMode.WALKING || travelMode === TravelMode.CYCLING) {
    throw new Error('Route optimization is only supported for DRIVING and TWO_WHEELER travel modes. ' + 'Walking and cycling routes cannot be optimized.');
  }
  ...
  if (waypoints.length > 25) {
    throw new Error('Google Routes API supports a maximum of 25 intermediate waypoints. ' + `Got ${waypoints.length}. Consider splitting into multiple routes.`);
  }
  ...
  const supportsRoutingPref = travelMode === TravelMode.DRIVING || travelMode === TravelMode.TWO_WHEELER;
  const body = {
    origin: {
      location: {
        latLng: {
  ...
  const optimizedIndices = route.optimizedIntermediateWaypointIndex ?? intermediates.map((_, i) => i);

  // Rebuild the waypoints array in the optimized order.
  // The last waypoint (destination) stays at the end.
  const reorderedWaypoints = [...optimizedIndices.map(idx => intermediates[idx]), lastWaypoint];

  // Build full optimized order (indices into original waypoints array)
  const fullOptimizedOrder = [...optimizedIndices, waypoints.length - 1];
