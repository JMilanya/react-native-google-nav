...
export function useBackgroundLocation(
  ...
  onLocation: (event: OnBackgroundLocationEvent) => void
): void {
  ...
  useEffect(() => {
    ...
    const subscription = emitter.addListener(
      'onBackgroundLocationUpdate',
      (event: unknown) => {
        onLocationRef.current(event as OnBackgroundLocationEvent);
      }
    );

    NativeGoogleNavModule.startBackgroundLocationUpdates(
      intervalMs,
      notificationTitle,
      notificationText
    )
